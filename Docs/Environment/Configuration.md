...
Unreal Engine supports platform-specific configuration files located in `Config/<Platform>/<Platform><Type>.ini`. These files override values in the base `Default<Type>.ini` for that specific platform.

### Linux Development Performance (`Config/Linux/LinuxEngine.ini`)

For development on Linux, we use a dedicated override file to ensure editor responsiveness and stability.

| Setting | Reason |
| --- | --- |
| `r.RayTracing=False` | Disables expensive ray tracing which often causes significant Slate UI lag on Linux. |
| `r.Shadow.Virtual.Enable=0` | Reverts to standard shadows to reduce GPU overhead in the editor. |
| `r.Nanite=0` | Temporarily disabled to ensure maximum stability across various Linux Vulkan drivers. |
| `TargetedRHIs=SF_VULKAN_SM5` | Forces Vulkan SM5 over SM6 for improved driver compatibility and stability. |

These settings prioritize a snappy, stable workspace over high-end visual fidelity during core simulation and logic development.
