...
FString LowerMegaKernel(
	const TMap<uint32, FVexProgramAst>& Scripts,
	const TArray<FVexSymbolDefinition>& SymbolDefinitions,
	const TMap<FString, FString>& SymbolMap,
	const FString& FunctionName,
	const TMap<uint32, TSet<FString>>* WrittenSymbolsByBehavior)
{
	...
	TSet<FString> StoreBackSymbols;
	for (const TPair<uint32, FVexProgramAst>& Pair : Scripts)
	{
		if (WrittenSymbolsByBehavior)
		{
			if (const TSet<FString>* WrittenSymbols = WrittenSymbolsByBehavior->Find(Pair.Key))
			{
				for (const FString& WrittenSymbol : *WrittenSymbols)
				{
					if (SymbolMap.Contains(WrittenSymbol))
					{
						StoreBackSymbols.Add(WrittenSymbol);
					}
				}
				continue;
			}
		...
		for (const FString& UsedSymbol : Pair.Value.UsedSymbols)
		{
			if (SymbolMap.Contains(UsedSymbol))
			{
				StoreBackSymbols.Add(UsedSymbol);
			}
		}
	}
	...
	if (StoreBackSymbols.Contains(TEXT("@position"))) Source += TEXT("    pos = position_local;\n");
	if (StoreBackSymbols.Contains(TEXT("@velocity"))) Source += TEXT("    vel = velocity_local;\n");
	if (StoreBackSymbols.Contains(TEXT("@shield")))   Source += TEXT("    shield = shield_local;\n");
	if (StoreBackSymbols.Contains(TEXT("@status")))   Source += TEXT("    status = (uint)status_local;\n");
