...
FString SSwarmOrchestrator::BuildReflectedTypeInclude(uint32& OutLayoutHash)
{
	...
	Source += ::Flight::Reflection::HLSL::GenerateStructContractHLSL<Flight::Swarm::FDroidState>(
		TEXT("FDroidState"),
		TEXT("FDROIDSTATE_LAYOUT_HASH"));
	Source += TEXT("\n");

	const FString ResourceContract = ::Flight::Reflection::HLSL::GenerateGpuResourceContractInclude<Flight::Swarm::FDroidState>(
		TEXT("FDROIDSTATE"));
	if (!ResourceContract.IsEmpty())
	{
		Source += ResourceContract;
		Source += TEXT("\n");
	}
