...
namespace
{
const TCHAR* FlightGeneratedShaderVirtualPath = TEXT("/FlightProject/Generated");
const TCHAR* FlightGeneratedShaderFileName = TEXT("VexMegaKernel.ush");
}

FString UFlightMegaKernelSubsystem::GetGeneratedShaderFilePath()
{
	return FPaths::ProjectIntermediateDir() / TEXT("Shaders/Generated") / FlightGeneratedShaderFileName;
}

FString UFlightMegaKernelSubsystem::BuildBootstrapMegaKernelSource()
{
	return TEXT("// Bootstrap generated by UFlightMegaKernelSubsystem\n")
		TEXT("#define VEX_MEGA_KERNEL_DEFINED\n\n")
		TEXT("void ExecuteVexBehavior(uint BehaviorID, inout float3 pos, inout float3 vel, inout float shield, inout uint status, float dt, uint frame)\n")
		TEXT("{\n")
		TEXT("    // Orchestration has not synthesized any GPU-bound behavior dispatch yet.\n")
		TEXT("}\n");
}

void UFlightMegaKernelSubsystem::EnsureGeneratedShaderBootstrap()
{
	const FString GeneratedDir = FPaths::GetPath(GetGeneratedShaderFilePath());
	IFileManager::Get().MakeDirectory(*GeneratedDir, true);

	if (!AllShaderSourceDirectoryMappings().Contains(FlightGeneratedShaderVirtualPath))
	{
		AddShaderSourceDirectoryMapping(FlightGeneratedShaderVirtualPath, GeneratedDir);
	}

	const FString FilePath = GetGeneratedShaderFilePath();
	if (!IFileManager::Get().FileExists(*FilePath))
	{
		if (!FFileHelper::SaveStringToFile(BuildBootstrapMegaKernelSource(), *FilePath))
		{
			UE_LOG(LogFlightProject, Error, TEXT("MegaKernel: Failed to write bootstrap shader include to %s"), *FilePath);
		}
		else
		{
			UE_LOG(LogFlightProject, Log, TEXT("MegaKernel: Wrote bootstrap shader include to %s"), *FilePath);
		}
	}
}
...
void UFlightMegaKernelSubsystem::Initialize(FSubsystemCollectionBase& Collection)
{
	...
	EnsureGeneratedShaderBootstrap();
